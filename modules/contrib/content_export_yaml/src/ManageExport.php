...
class ManageExport {
  ...
  public function getCount($entity_name,$bundle){
    $id_label = \Drupal::entityTypeManager()->getDefinition($entity_name)->getKey('id');
    $bundle_label = \Drupal::entityTypeManager()->getDefinition($entity_name)->getKey('bundle');

    $factory = \Drupal::entityTypeManager()->getStorage($entity_name)->getQuery();
    if ($bundle_label != "") {
        $factory->condition($bundle_label, $bundle);
    }
    return $factory->count()->execute();
  }
